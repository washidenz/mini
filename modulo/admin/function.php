<?php
	class fnAdmin
	{
		...
		function Usuarios()
		{
			...
			$search ='<div class="input-group input-group-sm" style="width: 250px;">
		...
		function ListaUsuario($Pag=1, $idUsuario=false)
		{
			...
			$query = "SELECT idUsuario,Usuario,Estado,Nombres,Paterno,Materno,email FROM Usuario LIMIT ".$desde.",".$this->Max.";";
			
			if( $idUsuario!=false )
			{
				$query = "SELECT idUsuario,Usuario,Estado,Nombres,Paterno,Materno,email FROM Usuario WHERE idUsuario=".$idUsuario.";";
			}
		...
		function Botonera( $idUsuario=false )
		{
			...
			$edit = '<button type="button" class="btn btn-success btn-xs OpenModal" data-destine="'.URL.MODULO.'/json/editausuario" data-target="#ModalPrincipal" data-data="'.$data.'"><i class="fa fa-edit"></i></button> ';
		...
		function GuardaUsuario( $datos )
		{
			...
			$rtn = array(
				"success"		=> false,
				"notification"	=> "ERROR: se a producido un error al crear el usuario."
			);
			return json_encode($rtn, JSON_PRETTY_PRINT);
		}
