<?php
	class Interfaz
	{
				...
				foreach( $resultado as $obj )
				{
					...
					$msj .= '<li>
								<a href="#">
									...
									<h4>
										'.$obj->Paterno.' '.$obj->Materno.', '.$obj->Nombres.'
		...
		public function get_navUserbar()
		{
			...
			{
				$idUsuario = $this->parents->session->get("idUsuario");
				$query = "SELECT idUsuario, Nombres, Paterno, Materno, Usuario, FotoPerfil, CreditoSMS, CreditoMail,((SELECT COUNT(*)FROM SMS INNER JOIN Suscrito ON SMS.idSuscrito = Suscrito.idSuscrito WHERE Suscrito.idUsuario = Usuario.idUsuario)+(SELECT COUNT(*) FROM NormalSMS WHERE NormalSMS.idUsuario = Usuario.idUsuario))GastoSMS, (SELECT COUNT(*) FROM Mail INNER JOIN Suscrito ON Mail.idSuscrito = Suscrito.idSuscrito WHERE Suscrito.idUsuario = Usuario.idUsuario)GastoMail FROM Usuario WHERE Usuario.idUsuario=".$idUsuario.";";
				if( $this->parents->sql->consulta($query) )
				{
					$resultado = $this->parents->sql->resultado;
					foreach( $resultado as $obj )
								...
								<li class="user-body">
									<div class="row">
										<div class="col-xs-6 text-center">
											<a href="#"><strong>SMS Enviados</strong></a>
											<p>'.$obj->GastoSMS.' de '.$obj->CreditoSMS.'</p>
										</div>
										<div class="col-xs-6 text-center">
											<a href="#"><strong>Email Enviados</strong></a>
											<p>'.$obj->GastoMail.' de '.$obj->CreditoMail.'</p>
										</div>
									</div>
								</li>
								<!-- Menu Footer-->
			...
			{
				$rtn='<li class="user user-menu">
					<a href="'.URL.'login">
						<i class="fa fa-user"></i><span class="hidden-xs">Inicie Sesion</span>
					</a>
				</li>';
			}
							...
							foreach($json->Menu as $m)
							{
								if( $this->parents->session->get_permission( $v["directorio"],$v["directorio"] ) )
								{
									...
									<li class="'.$issm.$active.'">
										<a href="'.URL.$v['directorio'].$m->URL.'/" '.$this->parents->fn->attribute($m->Attribute).'>
